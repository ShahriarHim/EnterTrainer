...
const Resource = () => {
    ...
    const handleAddOrUpdateResource = async () => {
        try {
            ...
            const response = await axios.post('http://localhost:5000/resources/addOrUpdateResource', resourceData);
            ...
            console.error('Error adding/updating resource:', error.response.data.error);
    ...
    const getAllResources = async (courseId) => {
        try {
            const response = await axios.get(`http://localhost:5000/resources/getAllResources/${courseId}`);
    ...
    return (
        <div style={{ display: 'flex', height: '100vh' }}>
            ...
            <div style={{ backgroundColor: '#333', color: '#fff', padding: '20px', width: '200px', height: '100%', position: 'fixed', left: 0, top: 0 }}>
                ...
                {userType === 'INS' && (
                    <button onClick={() => setShowAddResource(!showAddResource)} style={{ display: 'block', textAlign: 'center', backgroundColor: '#06BBCC', color: '#fff', padding: '10px', border: 'none', cursor: 'pointer' }}>
                        Add Resource
                    </button>
                )}
            </div>
                        ...
                        <div key={weekNumber}>
                            ...
                            {Array.isArray(weekResources[weekNumber]) ? (
                                weekResources[weekNumber].map((resource, index) => (
                                    <div key={index}>
                                        <p>Resource Link: <a href={resource.link} style={{ color: 'blue' }} target="_blank" rel="">{resource.link}</a></p>
                                    </div>
                                ))
                            ) : (
                                <div>
                                    <p>Resource Link: <a href={weekResources[weekNumber].link} style={{ color: 'blue' }} target="_blank" rel="">{weekResources[weekNumber].link}</a></p>
                                </div>
                            )}
                        </div>
                    ))}
